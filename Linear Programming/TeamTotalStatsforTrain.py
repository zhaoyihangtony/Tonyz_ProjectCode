...
for i in seasons:
    ...
    for index, row in pplosfile.iterrows():
        ...
        #print(S_PTS)
        B_AST.append(np.array(df2['AST']))
        B_STL.append(np.array(df2['STL']))
        B_BLK.append(np.array(df2['BLK']))
        B_FGM.append(np.array(df2['FGM']))
        B_FGA.append(np.array(df2['FGA']))
        B_3PM.append(np.array(df2['3PM']))
        B_3PA.append(np.array(df2['3PA']))
        B_FTM.append(np.array(df2['FTM']))
        B_FTA.append(np.array(df2['FTA']))
        B_TOV.append(np.array(df2['TOV']))
        B_PF.append(np.array(df2['PF']))
        B_OREB.append(np.array(df2['OREB']))
        B_DREB.append(np.array(df2['DREB']))
        B_REB.append(np.array(df2['REB']))
        B_DEF.append(np.array(row['DEFbackup']))
    ...
    dic = {'TEAM':teamname,
           'TotalS_PTS':TOTALS_PTS,
           'TotalS_AST':TOTALS_AST,
           'TotalS_STL':TOTALS_STL,
           'TotalS_BLK':TOTALS_BLK,
           'TotalS_FGM':TOTALS_FGM,
           'TotalS_FGA':TOTALS_FGA,
           'TotalS_3PM':TOTALS_3PM,
           'TotalS_3PA':TOTALS_3PA,
           'TotalS_FTM':TOTALS_FTM,
           'TotalS_FTA':TOTALS_FTA,
           'TotalS_TOV':TOTALS_TOV,
           'TotalS_PF':TOTALS_PF,
           'TotalS_OREB':TOTALS_OREB,
           'TotalS_DREB':TOTALS_DREB,
           'TotalS_DEF':TOTALS_DEF,
           'TotalB_PTS':TOTALB_PTS,
           'TotalB_AST':TOTALB_AST,
           'TotalB_STL':TOTALB_STL,
           'TotalB_BLK':TOTALB_BLK,
           'TotalB_FGM':TOTALB_FGM,
           'TotalB_FGA':TOTALB_FGA,
           'TotalB_3PM':TOTALB_3PM,
           'TotalB_3PA':TOTALB_3PA,
           'TotalB_FTM':TOTALB_FTM,
           'TotalB_FTA':TOTALB_FTA,
           'TotalB_TOV':TOTALB_TOV,
           'TotalB_PF':TOTALB_PF,
           'TotalB_OREB':TOTALB_OREB,
           'TotalB_DREB':TOTALB_DREB,
           'TotalB_DEF':TOTALB_DEF}
    vdf = pd.DataFrame(dic)
    print(vdf)
    vdf.to_csv(foldername+i+'TeamTotalstats(WITH_def).csv')
