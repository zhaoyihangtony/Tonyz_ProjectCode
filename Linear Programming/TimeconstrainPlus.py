...
def RidgeRegressionTeam(filename,testfile):
    ...
    Y=filename['WIN%']
    testY=testfile['WIN%']
    lm = linear_model.Ridge(alpha = 1.0,normalize=False)
    model = lm.fit(X,Y)
    Ridgemodelpredict = lm.predict(testX)
    w1 = lm.coef_[0]
    w2 = lm.coef_[1]
    w3 = lm.coef_[2]
    w4 = lm.coef_[3]
    ...
    #w9=lm.coef_[8]
    #w8=lm.coef_[8]
    return w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,lm.intercept_,Ridgemodelpredict
'''
...
if __name__ == '__main__':
    ...
    w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w0,linearPredict= RidgeRegressionTeam(traindata,testdata)
    ...
    #originalWinrate.to_csv(foldername+ 'Time/'+ '13-14_15-16_Ridge(Normalized)_Prediction_14attrs.csv')

    
    result=open(foldername + 'Time/' + "TimeConstrain_Ridge_RregressionWeight_14sttrs","w")
    ...
    for ppos in positionsCategory:
        ...
        prob+=lpSum(isIncluded[playerlist[index]]*Time[index] for index in playerindex) >= 40
        prob+=lpSum(isIncluded[playerlist[index]]*Time[index] for index in playerindex) <= 55
